...
# subdirs
################################################################
# single level subdirs
# subdirs:=$(shell cd $(TOP); ls -d */)
# subdirs:=$(patsubst %/,%,$(subdirs))
################################################################
# multi level subdirs (recursive)
...
demo:lib $(demo-objects)
	@echo $(subdirs-test)
	$(CXX) $(DEMO_CXXFLAGS) $(demo-objects) -o demo/demo $(DEMO_LDFLAGS)

run_demo:demo
	LD_LIBRARY_PATH=$(TOP)/$(LIB_EXPORT_DIR) ./demo/demo
